...
import {
  ...
  Search, LayoutGrid, Settings, CreditCard, TrendingUp,
  LogOut, Globe, MoreHorizontal, X,
} from 'lucide-react'
...
const nav = [
  ...
  { href: '/dashboard/import', label: 'Website → Pins', icon: Globe },
...
export function Sidebar() {
  ...
  return (
    <aside className="hidden md:flex fixed left-0 top-0 h-full w-14 lg:w-56 border-r border-gray-100 bg-white z-40 flex-col">
      {/* Logo */}
      <Link
        href="/dashboard"
        className="flex items-center gap-2.5 px-3 lg:px-4 h-16 border-b border-gray-100 shrink-0 min-w-0"
      >
        ...
        <span className="hidden lg:block font-semibold text-gray-900 text-sm tracking-tight truncate">Pinshedule</span>
      </Link>
      ...
      <div className="px-2 lg:px-3 pb-4 space-y-0.5 border-t border-gray-100 pt-3">
        ...
        <button
          onClick={logout}
          className="w-full flex items-center gap-3 px-3 min-h-[44px] rounded-xl text-sm font-medium text-gray-500 hover:bg-red-50 hover:text-red-600 transition-colors"
        >
...
export function MobileTabBar() {
  ...
  return (
    <>
      ...
      {/* Tab bar */}
      <nav
        className="md:hidden fixed bottom-0 inset-x-0 bg-white border-t border-gray-100 z-40 flex"
      ...
      >
        {mainTabs.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link
              ...
              href={href}
              className={cn(
                'flex-1 flex flex-col items-center py-2.5 text-[10px] font-medium transition-colors gap-0.5',
        ...
        <button
          ...
          className={cn(
            'flex-1 flex flex-col items-center py-2.5 text-[10px] font-medium transition-colors gap-0.5',
            moreOpen || isMoreActive ? 'text-[#E60023]' : 'text-gray-400'
          )}
        >
